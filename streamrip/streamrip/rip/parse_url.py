...
class SpotifyURL:
    """Represents a Spotify URL (track, album, playlist, or artist).

    Has no real download client — it is intercepted in ``Main.add()`` /
    ``Main.add_all()`` and routed to ``Main._make_spotify_pending()``, which
    scrapes the Spotify embed page and searches the result on the configured
    download service.
    """

    # Sentinel so callers can detect this type without importing it
    source = "_spotify"

    def __init__(self, url: str, media_type: str, spotify_id: str):
        self.url = url
        self.media_type = media_type   # "track" | "album" | "playlist" | "artist"
        self.spotify_id = spotify_id

    @classmethod
    def from_str(cls, url: str) -> "SpotifyURL | None":
        m = SPOTIFY_URL_REGEX.match(url)
        if m:
            return cls(url, m.group(1), m.group(2))
        return None
    ...
    async def into_pending(
        ...
        raise NotImplementedError(
            "SpotifyURL is handled by Main._make_spotify_pending(); "
            "into_pending() is not used.",
        )
